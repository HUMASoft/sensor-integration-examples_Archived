...
int main()
{
    ...
    for (int y=0;y<=5;y++){
        ...
        std::string DeviceName = s.substr(0,14);
        std::cout << "Device Name: " << DeviceName << '\n';
        int x=14, xinicial=14;

        std::string Acceleration_X;
        std::tie(Acceleration_X,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "Acceleration_X: " << Acceleration_X<<"\n";

        std::string Acceleration_Y;
        std::tie(Acceleration_Y,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "Acceleration_Y: " << Acceleration_Y << "\n";

        std::string Acceleration_Z;
        std::tie(Acceleration_Z,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "Acceleration_Z: " << Acceleration_Z << "\n";

        std::string AngularVelocity_X;
        std::tie(AngularVelocity_X,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "AngularVelocity_X: " << AngularVelocity_X << "\n";

        std::string AngularVelocity_Y;
        std::tie(AngularVelocity_Y,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "AngularVelocity_Y: " << AngularVelocity_Y << "\n";

        std::string AngularVelocity_Z;
        std::tie(AngularVelocity_Z,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "AngularVelocity_Z: " << AngularVelocity_Z << "\n";

        std::string Angle_X;
        std::tie(Angle_X,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "Angle_X: " << Angle_X << "\n";

        std::string Angle_Y;
        std::tie(Angle_Y,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "Angle_Y: " << Angle_Y << "\n";

        std::string Angle_Z;
        std::tie(Angle_Z,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "Angle_Z: " << Angle_Z << "\n";

        std::string MagneticField_X;
        std::tie(MagneticField_X,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "MagneticField_X " << MagneticField_X << "\n";

        std::string MagneticField_Y;
        std::tie(MagneticField_Y,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "MagneticField_Y: " << MagneticField_Y << "\n";

        std::string MagneticField_Z;
        std::tie(MagneticField_Z,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "MagneticField_Z: " << MagneticField_Z << "\n";

        std::string ModuleTemperature;
        std::tie(ModuleTemperature,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "TemperatureModule: " << ModuleTemperature << "\n";

        std::string ModuleDepth;
        std::tie(ModuleDepth,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "DepthModule:  " << ModuleDepth << "\n";

        std::string AirPressure;
        std::tie(AirPressure,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "AirePressure:  " << AirPressure << "\n";

        std::string ModuleHeight;
        std::tie(ModuleHeight,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "HeightModule:  " << ModuleHeight << "\n";

        std::string q0;
        std::tie(q0,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "Q0:  " << q0 << "\n";

        std::string q1;
        std::tie(q1,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "Q1:  " << q1 << "\n";

        std::string q2;
        std::tie(q2,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "Q2:  " << q2 << "\n";

        std::string q3;
        std::tie(q3,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "Q3:  " << q3 << "\n";

        std::string PortD1;
        std::tie(PortD1,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "Port1:  " << PortD1 << "\n";

        std::string PortD2;
        std::tie(PortD2,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "Port2:  " << PortD2 << "\n";

        std::string PortD3;
        std::tie(PortD3,x)=ReadFromDataString(xinicial,x,s);
        std::cout << "Port3:  " << PortD3 << "\n";

        std::string PortD4;
        std::tie(PortD4,x)=ReadFromDataString(xinicial,x,s);
